    ...
    <div className="flex">
      ...
      <div className="flex flex-col items-center px-10 pt-10 w-5/6">
        <header className="w-full">
          <h1 className="text-5xl font-bold text-center">Welcome to Your Movie Watchlist</h1>
        </header>
        
        <p className="text-2xl text-center mt-6">Track and organize movies you want to watch!</p>

        <form onSubmit={handleSearch} className="mt-20 w-full max-w-2xl mx-auto flex items-center">
          <input
            type="text"
          ...
          <div className="mt-20 w-full">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {recentMovies.map((movie) => (
                <div key={movie.imdbID} className="w-1/5 p-2 cursor-pointer" onClick={() => navigate(`/movie/${movie.imdbID}`)}>
                  <img src={movie.Poster} alt={movie.Title} className="rounded-lg shadow-lg aspect-[3/4]" />
                  <p className="text-sm mt-2 text-center">{movie.Title} ({movie.Year})</p>
                </div>
              ))}
            </div>
          </div>
